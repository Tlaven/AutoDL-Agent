...
@pytest.mark.asyncio
async def test_supervisor_e2e_smoke_single_dialog(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class FakeModel:
        ...
        async def ainvoke(self, _messages: list) -> AIMessage:
            ...
            return AIMessage(content="已完成并给出总结。", tool_calls=[])
    ...
    result = await graph_module.graph.ainvoke(
        {"messages": [HumanMessage(content="请规划并执行一次任务")]},
        context=Context(model="qwen:qwen-flash", system_prompt="sys"),
    )

    assert result["planner_session"] is not None
